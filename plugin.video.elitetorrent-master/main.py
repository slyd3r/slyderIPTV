...
def index():
    ...
    items = [
        {'label': settings.string(32194),
         ...
         'thumbnail': dirImages("busqueda_manual.png"),
    ...
    items.append({'label': plugin.get_string(32017),
...
def search():
    ...
    url = "/busqueda/%s/pag:" % query.replace(" ", "+")
    response = settings.dialog.yesno(settings.name, settings.string(32193))
    if response:
        name = ''
        while name is '':
            name = settings.dialog.input(plugin.get_string(32193)).title()
...
def nextPage(url="", page="1", showSeasons='True'):
    urlSearch = settings.value["urlAddress"] + url + page
